...
namespace LM {
	...
	private:
		...
		// Weapon state
		uint64_t		m_last_fired_time;

	public:
		StopGun(const char* name, int damage, uint64_t delay, double recoil);
		explicit StopGun(PacketReader& gun_data);
